...
sub generalConsulta {
	...
	# Cargo los archivos maestros, para las validaciones de los filtros
	my %Emisores = ();
	&cargoEmisores;
	my %Gestiones = ();
	&cargoGestiones;
	...
	# Primero llamo a los modulos de los filtros.
	print
"En todos los casos, si no se ingresa elemento en el filtro se buscara por todos los elementos\n";
	print
"En el caso que deseara hacer una busqueda por rangos, debe ingresar los valores separados por un -\n";
	print "Ingrese el año o rango de años: \n";
}

sub realizoConsulta {

	# Primero llamo a los modulos de los filtros.
	my $filtroTiposNorma = &filtroNorma;
	my @filtroAnios      = &filtroAnio;
	my @nroNorma         = &filtroNroNorma;
	my $gestion          = &filtroGestion;
	my $emisor           = &filtroEmisor;
...
sub procesoConsulta {
	local %hashTemp = ();
	&procesoDirectorioConsulta( substr $PROCDIR, 0, -1 );
...
sub procesoArgumentos() {
	...
	for ( my $cont = 0 ; $cont < $largo ; $cont++ ) {
		...
		if ( substr $argumentos[$cont],1,1 eq "w" ) {
			$w = 1;
			next;
		}
